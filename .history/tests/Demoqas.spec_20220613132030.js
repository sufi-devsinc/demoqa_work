...
const {Interactions, interactions } = require('./demo.js/Interaction')
const { main_pages, main_pages } = require('./demo.js/main_page');
// Json Files
...
test.describe('demoqa tests', async () => {
    ...
    test(`@Selectable interaction with selectable`, async ()=> {
        const interaction_1 = new interactions(page);
        await interaction_1.initialize();
        await page.pause();
        await interaction_1.Selectable();
    });
    test (`@Tool_SQA bookastore with toolSqa`, async ()=> {
        const main_page_1 = new main_pages(page);
        await main_page_1.initialize();
        await page.pause();
        await main_page_1.Tool_SQA();
    });
